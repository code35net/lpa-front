...
const ListToolbar = () => {
  ...
  const openAddModal = () => {
	...
	const handleDownloadClick = async () => {
        getActions("").then((res) => {
            const ar = [intl.formatMessage({ id: 'Id' }), intl.formatMessage({ id: 'ACTION.LIST.COL.FIND' }),
                intl.formatMessage({ id: 'LASTDATE' }), intl.formatMessage({ id: 'ACTION.LIST.COL.STAFF' }),
                intl.formatMessage({ id: 'ANSWER' }), intl.formatMessage({ id: 'ACTION.LIST.COL.STATUS' }),
                intl.formatMessage({ id: 'CloseDate' }), intl.formatMessage({ id: 'IsLate' }),]
            const data2 = res.data?.reduce((result: any, item: any) => {
                ...
                result.push([item.id, item.finding, item.lastDate, item.assignedUserName,
                    item.text, status, item.endDate, islate,]);
                return result;
            }, []);
  ...
  return (
      <div className='d-flex justify-content-end' data-kt-item-table-toolbar='base'>
       ...
       <button className='btn btn-sm btn-white btn-active-dark me-3'
              onClick={handleDownloadClick}><KTSVG path='/media/icons/duotune/arrows/arr078.svg' className='svg-icon-2' />{intl.formatMessage({ id: 'TOOLBAR.EXPORT' })}</button>
