...
const THING_URL = `${API_URL}/AuditChange`
const UPDATE_THING_URL = `${API_URL}/Custom/auditChange`

const getThings = (query: string): Promise<QueryResponse> => {
  return axios
    .get(`${THING_URL}/getAll?${query}&modelstoinclude=Audit.Unit,Unit`)
...
const updateThing = (thing: Model): Promise<Model | undefined> => {
  return axios
    .post(`${UPDATE_THING_URL}/${thing.id}`, thing)
    .then((response: AxiosResponse<Response<Model>>) => response.data)
    .then((response: Response<Model>) => response.data)
}
