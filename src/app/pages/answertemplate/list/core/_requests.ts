...
const listAnswerTemplates = async (): Promise<any> => await axios.get(`${ANSWERTEMPLATE_URL}/getAll?page=1`).then((res : AxiosResponse) => 
 {
   return res.data;
 });
...
const getAnswerTemplates = (query: string): Promise<QueryResponse> => {
  return axios.get(`${ANSWERTEMPLATE_URL}/getAll/?${query}`).then((d: AxiosResponse<QueryResponse>) => d.data)
}
