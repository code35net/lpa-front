...
const Columns: ReadonlyArray<Column<Model>> = [
  ...
  {
    Header: (props) => <CustomHeader tableProps={props} title={useIntl().formatMessage({id: 'HOLIDAYS_NAME'})} className='min-w-125px' />,
    id: 'whatDay',
    Cell: ({...props}) => <InfoCell item={props.data[props.row.index].whatDay} />,
  },
  {
    Header: (props) => <CustomHeader tableProps={props} title={useIntl().formatMessage({id: 'HOLIDAYS_DATE'})} className='min-w-125px' />,
    id: 'theDay',
    Cell: ({...props}) => <InfoCell item={moment(props.data[props.row.index].theDay).format('YYYY-MM-DD')} />,
  },
