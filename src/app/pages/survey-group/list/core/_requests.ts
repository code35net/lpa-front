...
const THING_URL = `${API_URL}/SurveyGroup`



const getThings = (query: string): Promise<QueryResponse> => {
  const qsd = qs.parse(window.location.search, { ignoreQueryPrefix: true }).surveyId
  return axios
  .get(`${THING_URL}/getAll/SurveyId-${qsd}?${query}&modelstoinclude=Survey`)
  .then((d: AxiosResponse<QueryResponse>) => d.data)
}

const listThings = async (surveyId: number): Promise<any> =>
  await axios.get(`${THING_URL}/getAll/SurveyId-${surveyId}?page=1&items_per_page=999`).then((res: AxiosResponse) => {
    return res.data
  })
...
const createThing = (thing: Model): Promise<Model | undefined> => {
  const qsd = qs.parse(window.location.search, { ignoreQueryPrefix: true }).surveyId ?? "1"
  thing.surveyId = parseInt(qsd?.toString())
  return axios
    .put(THING_URL, thing)
    .then((response: AxiosResponse<Response<Model>>) => response.data)
